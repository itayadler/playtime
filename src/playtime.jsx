...
class Playtime extends Component {
    ...
    if (input === null) {
      ...
      if (input !== null || input !== "" || input !== "NaN") {
        ...
        setInterval(() => {
          ...
          this.state.countDownLastMinute
            ? this.setState({ playTimeCountdown: seconds })
            : this.setState({ playTimeCountdown: minutes + "m" });
          if (
            minutes <= timeCloseToFinish &&
            !this.state.countDownCloseToFinish
          ) {
  ...
  renderVideoThumbnail = (url, label, index, listName) => {
    return (
      <div key={index} className="tofu__list--item">
        <div
          className={
            "tofu__list--item--container " +
            (this.state.editMode ? "wiggle" : "") +
            (this.checkRecommended(url) ? "tofu__list--item--recommended" : "")
          }
        ...
        <div
          className={
            "tofu__list--item--deleteBtn " +
            (this.state.editMode ? " " : "displayNone")
          }
          onClick={() => this.handleArchiveVideo(url, listName)}
        ></div>
      </div>
    );
  };
  renderList = (list, categoryName, colorClass, listName) => {
    ...
    return (
      <div
        className={
          "tofu__rightMenu--list " +
          colorClass +
          (isArchive && !this.state.editMode ? " displayNone" : " ")
        }
      >
        <div className="tofu__rightMenu--list--title-container">
          ...
          <div
            className={
              "controls__btn controls__btn--pencil tofu__rightMenu--list--btn " +
              (this.state.editMode ? "fade-in" : "displayNone")
            }
            onClick={() => this.handleEditTitle(categoryName, listName)}
          ></div>
        </div>
        <div className="tofu__rightMenu--list--container">
          ...
          <div className="tofu__list--item">
            <div
              className={
                "controls__btn controls__btn--plus tofu__list--item--container tofu__list--item--plus " +
                (this.state.editMode ? "fade-in" : "displayNone")
              }
              onClick={() => this.handleAddVideo(listName)}
            ></div>
            <div className="tofu__list--item--holder"></div>
          </div>
        </div>
  ...
  overlayMainVideo = () => {
    return (
      <div>
        <div
          id="overlay"
          className={" " + (this.state.playing ? "showOnHover " : "")}
          onMouseMove={this.handleMouseMoveOverVideoTimout}
        >
          <div
            className={
              "overlay__bg " + (this.state.playing ? "" : "overlay__bg--pause")
            }
          ...
          <div className="overlay__header">
            <div
              className={
                "logo__container " +
                (this.state.countDownCloseToFinish ? "wiggle" : "")
              }
              onClick={this.handlePlayTime}
            >
              <div className="logo__symbol">
                <div
                  className={
                    "logo__symbol--play-icon " +
                    (this.state.playTimeCountdownON ? "displayNone" : "")
                  }
                ></div>
                <p
          ...
          <div
            ...
            className="iconVideoBabu iconVideoBabu__L fade-in-out shape-nonagon"
            onClick={this.handlePlayPause}
          >
            <div
              className={
                "iconVideoBabu__icon " +
                (this.state.playing ? "shape-play" : "shape-pause")
              }
            ></div>
          </div>
          <div className="overlay__bottom">
            ...
            <div className="overlay__bottom--center-container">
              <div
                ...
                onClick={this.handleJumpBackwards}
              ></div>
              <div
                className={
              ...
              <div
                ...
                onClick={this.handleJumpForward}
              ></div>
            </div>
            <div
              className={
                "controls__btn controls__btn--arrow shape-circle " +
                (this.state.rightMenu ? "" : "flip-vertical")
              }
  ...
  render() {
    ...
    return (
      <div>
        <div
          id="playTimeOver"
          className={this.state.playTimeOver ? " " : "displayNone"}
        >
        ...
        <div id="tofu" className={this.state.playTimeOver ? "displayNone" : ""}>
          <div className="tofu__container">
            ...
            <div
              className={
                "tofu__rightMenu fade-in " +
                (this.state.rightMenu ? "" : "hide")
              }
            >
              <div className="tofu__rightMenu--nav">
                <div
                  className={
                    "controls__btn controls__btn--edit " +
                    (this.state.editMode
                      ? "controls__btn--edit--on"
                      : "controls__btn--edit--off")
                  }
              ...
              <div className={"tofu__rightMenu--lists"}>
                {this.renderList(
                  ...
                  "rightMenu__list--yellow",
                  "list000"
                )}
                {this.renderList(
                  this.state.list001,
                  this.state.categories[1],
                  "rightMenu__list--orange",
                  "list001"
                )}
                {this.renderList(
                  this.state.list002,
                ...
                {this.renderList(
                  ...
                  "rightMenu__list--purple",
                  "list003"
                )}
                {this.renderList(
                  this.state.list005,
                  this.state.categories[5],
                  "rightMenu__list--green",
                  "list005"
                )}
                {this.renderList(
                  this.state.listArchive,
                  "Archive",
                  "rightMenu__list--black",
                  "listArchive"
                )}
              </div>
            </div>
